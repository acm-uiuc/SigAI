...
class Agent():
    ...
    def train_policy_net(self, frame):
        ...
        rewards = torch.from_numpy(np.array(rewards)).to(device).float()
    
        # Compute Q(s_t, a) - Q of the current state
        ### CODE ####
        Q = self.policy_net(states)
        selected = Q[range(len(states)), actions]
        
        # Compute Q function of next state
        ### CODE ####
        Q_next = rewards + self.discount_factor * torch.max(self.target_net(next_states).detach(), dim=1).values
        Q_next[dones==1] = rewards[dones==1]
        # Find maximum Q-value of action at next state from target net
        ### CODE ####
        
        # Compute the Huber Loss
        ### CODE ####
        loss = torch.mean((selected - Q_next) ** 2)
